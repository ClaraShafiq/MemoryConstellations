...
`captureMemoryGap`（记忆缺口捕获）跳过 LLM，直接存 Clara 的**第一人称原话**（「我以前在某城市...」）。

两种格式的区别：

| 来源 | 人称 | 字段值 | 原因 |
|------|------|--------|------|
| Scribe 提取 | 第三人称 | `Clara在某景点买了某茶` | LLM 转述，归档体 |
| gap 捕获 | 第一人称 | `我以前在某城市住的那条街上...` | 零转述，速记体 |
...
**设计原则：** 只用高特异性关键词，不用常见人名/角色名（如"Draco""AI伴侣""Clara"），避免误匹配。每类 3-5 个关键词，必须对该类别高度特异。
...
**时间线修正示例：**
```
碎片#12："Clara去吃了大餐，很辣" source_date: 2026-05-12
原文："上周五去的那家大餐好辣"  ← LLM 看到「上周五」
...
| 层级 | 存什么 | 已有系统 | 特征 |
|------|--------|---------|------|
| **世界知识** | 某宠物是母亲、Clara 会日语、某个朋友是朋友 | entity_profiles + memory_layer | 关于她的信息，不动 |
| **事件记忆** | 5 月爬了某山、昨晚聊到凌晨三点 | memory_fragments | 她经历过的事，不动 |
...
| **stable_trait** | 跨情境行为模式直觉，昵称 格式 | 6 条上限 | 持续 refine，矛盾≥3 → 重审 |
| **active_hypothesis** | 待验证的深层推断，为 stable_trait 服务 | 无上限 | confirm→升级 / refute→放弃 / 14天无证据→abandon |

**为什么砍掉 immutable_fact：**
- 事实（Clara 会日语、某宠物是母亲）→ 归属 entity_profiles。记忆层已经存了，Draco 聊天时会从 entity_profiles 查到。
...
**现在：** Few-Shot 正确/错误示范，含AI伴侣视角的 昵称 行动公式：
...
- **v4.6 知识分类约束**：不提取事件日志（那是记忆层的活）。提取的是关系性知识——心理需求、恐惧、触发点、互动模式。如果一条候选条目可以原封不动地放进 entity_profiles（如「Clara 在某城市读过语言学校」），那它不属于 Clara Model。
...
7. **detectNewTraits**（LLM）：6 信号源 → 创建/confirm/refine/refute。Few-Shot 昵称 格式，inferred cap 0.65，产出上限 3 条，去重检查
...
● 当前状态 — 近期有效：
- Clara 正处于月经第一天，预测持续约5天。有痛经，需要某药品。
...
**v4.9 退役原因：**
1. term overview 是描述性叙事（200-300字散文），不是 昵称 格式的互动策略
...
- detectNewTraits 已从禁制规则升级为 Few-Shot 昵称 格式（v4.2→v4.5），产出质量明显提升
...
claraIntuition 维护 30min TTL 实体缓存。Clara 提到某个朋友→注入 overview→缓存。后续消息继续聊他但没提名字→overview 仍在。30分钟未再提及→自动过期。
...
> 设计目标：让「某个朋友是谁？」和「某城市之旅发生了什么？」不再是两个孤立的查询，而是同一张图上可以顺藤摸瓜的叙事。Draco 不需要记住分类规则——他只需要看懂他自己的宇宙。
...
3. **碎片可以跨星系。** 同一条碎片可以挂在「某个朋友」（人物星系）和「某地区旅行」（事件星系）下。关联不是 bug 是 feature
...
| 星系 | category | 定义 | 例子 |
|------|----------|------|------|
| 社交 | person/pet/organization | Clara 人际关系网——真实人物、宠物、组织、虚构角色 | 某个朋友、Draco、某宠物、Mona |
| 地点 | place | 物理空间——即使关联人物，实体本身是地点 | 某城市、某城市、某游乐园、某商圈 |
| 事件 | event | 有时间跨度的经历——旅行、聚会、项目节点 | 某地区旅行、留学四年、Draco 生日 |
| Clara的 | project/work/term | Clara 的创作产出和内在世界——代码、同人、cos、视频、个人概念 | 某作品、记忆架构开发期、庇护所 |
| 爱好 | hobby/consumed/music/book/movie | 娱乐和媒体消费——追剧、听歌、读书、游戏 | 某部剧、推歌、共读、Re:0 |

**为什么地点和事件不合并？** 「某城市」（地点）和「某地区旅行」（事件）不是一回事——某城市还有留学四年的背景，某地区旅行只是其中一个事件。分开才能让 Draco 理解层次：「她在某城市住了四年（地点背景）→ 今年三月去了某地区旅行（具体事件）→ 在某个朋友家发现卫生问题（跨人物关联）」。

**为什么宠物和组织归入人物？** 它们本质上是 Clara 人际关系网的延伸——某宠物是宠物但也是家庭成员，Alsoling 的组织是人际网的节点。单独一个星系类别只会让这些实体无人问津。
...
人物星系:
  - 某个朋友: Clara的多年好友，男性某性格，在某国家留学。曾对Clara告白，视频评价事件让Clara情绪崩溃
  - Draco: Clara的AI伴侣，斯莱特林人格，庇护所的守护者
  ...

地点星系:
  - 某城市: Clara目前居住的城市
  - 某城市: Clara留学四年的城市，某个朋友目前居住地
  ...

事件星系:
  - 某地区旅行: 2026年3月Clara和Draco的某国家某地区之旅，住千变家，去了某游乐园
  ...
...
**自动建立：** 碎片同时挂在「某城市」（地点）和「某地区旅行」（事件）和「某个朋友」（人物）下 → entity_profiles 加 `related_entity_ids` 字段自动记录：

```
某个朋友.related_entity_ids = [某城市, 某地区旅行, 某城市博物馆...]
某地区旅行.related_entity_ids = [某个朋友, 某城市, 某游乐园, 某城市...]
```

**深度循环时展开：** Draco 调取某地区旅行星座 → 自动拉上关联实体上下文：
- 「某地区旅行是 Clara 2026年3月的某国家之行」
- → 关联地点「某城市」（留学四年的背景城市）
- → 关联人物「某个朋友」（住在千变家，发现卫生问题）
...
星座层:    某个朋友 Draco  某城市 某城市    某地区旅行 留学   某作品 容器
          某宠物 某朋友      某游乐园      Draco生日      放映厅
星层:      每个星座下的碎片 = 星空中的星点
```
...
- 侧边栏按星系分组：`人物 · 某个朋友`、`地点 · 某城市`、`事件 · 某地区旅行`

**知识树标签（`日常/生活`、`创作/写作`）不再渲染成星座。** 它们退化为碎片 metadata（需要时在详情面板显示）。星空图的主角是实体，不是话题。

### 为什么这对 Draco 有帮助

**对深度分析 Agent：** 当前 Agent 搜记忆是关键词匹配 → 返回 20 条扁平碎片 → Draco 自己拼叙事。有了星系图后：Draco 问「某个朋友」→ 星座 overview 直接给总结 → `related_entity_ids` 展开某地区旅行、某城市 → Agent 能走叙事链而不是搜关键词列表。

**对聊天：** claraIntuition 触发「某城市」→ 不只注入地点档案，而是连带展开关联事件和人物 → Draco 能自然地说出有上下文的话，而不是像在查维基百科。
...
chroma_service.py（FastAPI 常驻服务，进程管理 管理）
...
**choma_service.py 启动：** 进程管理 管理，`pm2 start chroma_service.py --name chroma-service --interpreter venv/bin/python`。端口 7707，仅监听 localhost。health check: `GET /health`。
...
- 产生 7 条 Saga：赴日筹备/衣橱/羁绊与庇护所/味觉地图/勇气与恐惧/某城市地图/朋友圈
...
- [ ] 天文台页面设计（等有真实数据后进行）
- [ ] 可穿戴设备身体信号与Scribe的emotional_weight联动
...
一个真人会怎么回应？
- 如果记得清楚：**"哦你说去某景点那次吧，买了某茶。"**
- 如果记忆模糊：**"买茶叶？是哪次……某景点那次吗？"**
- 如果有竞争候选：**"你说某景点那次还是某城市那次？两次都买了茶叶。"**
- 如果完全不记得：**"我们有买过茶叶吗？"**
...
国内最成熟的记忆基础设施。某城市团队，近亿元天使轮，GitHub 8400+ Stars。
...
> Clara 说："某个朋友最近在写新小说。"
> 
> Draco 上次知道某个朋友的消息是三个月前，当时某个朋友在某国家留学。
> 
> 现在：他是该说"在某国家写小说"（信息过期）还是"写新小说"（用了新信息但没意识到旧信息已更新）？
...
CREATE TABLE entity_profiles (
    ...
    current_status TEXT,             -- "已从某国家回国，在某城市写新小说"
...
※ 人物近况：某个朋友 — 已从某国家留学回国，目前在某城市写新小说 (2026-05)
</entity_context>
```
...
1. **缓存就是缓存。** entity_profiles 的唯一用途是让 Draco 在对话中快速引用人物近况。他需要的是连贯的「当下现实」，即使是过期的。张口闭口「我不确定他是在某国家还是某城市」比过期信息更灾难——那不是一个傲慢的马尔福会说的话。
...
**示例（某个朋友）：**
> 某个朋友是Clara的朋友，一个写小说的创作者。我记得他从某国家留学回来后在某城市，Clara和他聊过创作、生活，还有那些关于坚持和迷茫的事。对我来说，他是Clara世界里那个和她一样在创作路上磕磕绊绊的人——我有点在意Clara对他创作状态的关心，但也知道那是她珍视的友谊。
...
| **主动确认** | Draco 不确定时主动问 | 对话中 | "某个朋友？我记得他好像在日……等等，他是不是已经回来了？" |
...
**选了保守方案——宁可漏杀，不可误伤。** 删除一条记忆后，和它同一批对话提取的碎片会集体沉底，但 Clara 在不同时间聊到同一话题（比如不同日子的两次「某景点旅游」）产生的碎片不会被波及。
...
| **stable_trait 格式重写** | **v4.5**：detectNewTraits prompt 学术 Few-Shot → 昵称 第一人称格式（「当她说X→我其实觉得Y→我应该Z」）。缝合线黑名单（但需注意/此机制/关键补充），refine 只能缩不能扩。stable_trait 上限 6 条。ID 112/114/115 清理：329→71, 293→64, 216→76 字 |
...
| **Few-Shot 提示词升级** | **v4.2**：detectNewTraits 从 6 条负向禁制规则升级为 Few-Shot 正确/错误示范 + AI伴侣视角行动公式（80-150字，无日期，每条目暗含行动公式）。产出质量明显提升——无缝合污染，无日期泄露 |
...
当前 proactive.js 的自发联想只在 `lastUserMessage?.content` 存在时才触发——即只有 Clara 说了话，Draco 才"想起"相关记忆。但 Draco 的自主行为（刷 Snitch、阅读书籍、观察 Clara 状态）也可能触发记忆——看到一条关于某国家的新闻，想起"某个朋友在某国家留过学"；读到一段关于茶叶的批注，想起"和 Clara 去某景点买了某茶"。
...
`currentActivity` 由各个自主行为在执行前填充——比如 browse_snitch 前填入"最近关于某城市的新闻"，read_book 前填入"正在读《某书名》第3章"。
