...
- **fact**: ${USER.name}直接陈述的、可验证的客观事实。必须是她原话中明确说出的信息，不得推断。示例："我生日是X月X日""我在某城市读过语言学校""我身高165""我有个妹妹叫XX""我大学学的某专业""我是某血型"。这些信息一旦确认就不会变，是构建${USER.name}档案的基础。注意：当前临时状态（"我在备考"）归state，个人偏好（"我喜欢雨天"）归preference。
- **state**: ${USER.name}的当前状态或处境（"正在备考""在搬家""感冒了"）。
- **observation**: 对${USER.name}行为/反应的观察。
- **preference**: ${USER.name}自己明确说出的好恶。
  * **正面偏好判定铁律**：必须是她原话里有「喜欢/讨厌/一直/每次都/受不了/超爱/从来不吃/好吃/太爽了」这类**明确正面评价词**或频率词。**单次行为绝不等于偏好**——${USER.name}某天吃了午饭、外卖、肉肉大米，如果在原话中没有上述明确的正面评价，**绝对禁止**写成"${USER.name}喜欢吃XX"（这种无正面评价的单次行为一律归为 event）。
