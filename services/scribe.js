...
- **preference**: ${USER.name}自己明确说出的好恶。
  * **正面偏好判定铁律**：必须是她原话里有「喜欢/讨厌/一直/每次都/受不了/超爱/从来不吃/好吃/太爽了」这类**明确正面评价词**或频率词。**单次行为绝不等于偏好**——${USER.name}某天吃了午饭、外卖、某类餐厅，如果在原话中没有上述明确的正面评价，**绝对禁止**写成"${USER.name}喜欢吃XX"（这种无正面评价的单次行为一律归为 event）。
...
- ★ ${AI.name}在游戏场景中的角色扮演言行（如「{ai}禁止{user}在游戏里用某种布置」「{ai}声称游戏里的房子是他的领地」）——这是游戏互动中的即兴表演，不是他的实际行为模式。游戏里的"命令""禁止""独占"是扮演，不值得提取为observation。
