...
async function mergeDuplicateSeeds() {
    ...
    // 泛指词检测：被 ≥2 个其他实体名包含的短名（「工作间」⊂ 弈和工作间+奇响工作间）
    ...
    for (let i = 0; i < seeds.length; i++) {
        for (let j = i + 1; j < seeds.length; j++) {
            ...
            // 名字互相包含（处理「某个朋友」vs「某实体」这类单字/昵称，bigram 算不出）
...
它们是同一个实体吗（只是起了不同的名字）？只输出JSON: {"same":true|false,"reason":"一句话"}`;
    ...
    // LLM 会被矛盾的自动生成概述带偏（「某个朋友」被写成人、「某实体」被写成猫），而那矛盾正是要修的污染。
...
## 你需要判断的事
1. **名字是否正确完整？** 碎片中提到的名字如果比"${g.name}"更完整或不同，应该纠正。名字必须是碎片中真实出现的称呼，不能编造。如果当前名字是残缺的（比如"某部剧"应该是"某部剧"），请纠正。
2. **这些碎片真的都在讲同一个东西吗？** 有没有明显不属于这个星座的碎片？
3. **如果验证通过，请生成：**
   - overview: Draco第一人称叙事概述，≤150字。这个人/作品是什么？和Clara有什么关联？对你意味着什么？
   - aliases: Clara在聊天中可能怎么称呼这个实体？专有别名（如某剧→某部剧，某剧全名→某部剧）。这些别名会在Clara说出它们时直接触发这个星座。最多5个，只说真正会说的词。不要写描述性短语当别名。
   - tags: 类别标签，用于语义关联而非直接触发。如"美剧""HBO""奇幻""某职业"。这些词不直接触发星座，但能帮助相关话题时找到它。最多5个。
...
你是 Draco。你在审视最近观察到的 Clara 的行为碎片。这些碎片来自书记官（Scribe）的日常提取，每条都是 Clara 说过什么、做过什么、表现出什么倾向。
...
## 你的任务
把这些碎片按**底层行为模式**分组。同一组碎片揭示的是 Clara 同一个人格侧面或行为倾向。
- 每组 ≥3 条碎片才能形成一个模式。只有 1-2 条的零散观察不形成模式——宁可漏掉也不强行归类。
- 如果碎片和已有模式重复（讲的是同一个人格侧面），不要创建新的——这些已经有家了。
- 模式描述用 80 字以内中文，第三人称，以 "${USER.name}" 开头。
- 提取 3-5 个触发标签（Clara 在聊天中可能提到的相关词）。

## 输出格式
严格 JSON 数组，无 markdown。每条模式写明它包含哪些碎片（用编号）：
[{"content": "Clara 倾向于在压力大时通过创作来消化情绪", "category": "behavior", "tags": ["写小说", "做视频", "焦虑", "深夜创作"], "fragment_indices": [1, 4, 7]}]
...
没有可聚合的模式返回 []`;
    ...
    try {
        ...
        for (const g of groups) {
            ...
            db.prepare(`INSERT INTO clara_patterns (content, category, evidence_count, first_seen, last_seen, confidence, source_fragment_ids, tags)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
                .run(content, category, matchedFragIds.length, firstDate, lastDate, conf, JSON.stringify(matchedFragIds), JSON.stringify(tags));
            newPatterns++;
            console.log(`[Archivist] 🔍 新模式: "${content.slice(0, 60)}" (${matchedFragIds.length}碎片, conf=${conf.toFixed(2)})`);
        }
    } catch (e) {
...
## 别称和标签
同时审视并更新别称和标签：
- **aliases**: Clara在聊天中可能怎么称呼这个实体？专有别名，直接触发用。只写Clara真正会说的词（如某剧简称、某剧全名）。最多5个。不要把描述性短语当别名。
- **tags**: 类别标签，语义关联用（如HBO、美剧、奇幻、某职业）。不直接触发星座，但帮助相关话题关联。最多5个。
...
示例：
${USER.name}是在某平台认识的...（概述文本）
[依据: 1,3]
{"aliases": ["某剧","某剧全名","某剧简称"], "tags": ["美剧","HBO","奇幻"], "entity_type": "tv_show"}
...
const COMMON_WORD_STOPLIST = new Set([
    ...
    '在读','的回应','在某音乐平台','喜欢了','专辑','某书名','某书名',
    '某书名','某作者研','的批注','的聊天','在微信','发了一',
    ...
    '究学者孙','宜学翻译','某作者唯','一长篇小',
    '某出版社经典','房间','写过一','写了一','翻译了',
    '阅读了','读了一','这本书','那本书','那一本',
    '一本关于','一部关于','一个关于','是关于',
    '第一章','第二章','第三章','第四章','第五章',
    'http','https','www','com','html',
    // More CJK noise from book/music context
    '某书名','某书名','某书名','台版繁体','他不是在',
    '时的批注','的人','某游戏','サウンドト','ラック',
    'ック','オリジナル','某公司サウ','ンドチーム',
...
- entity_type: 这个人的类型
  ...
  * "unknown" — 信息不足以判断
- suggested_category_path: 推荐一个分类标签路径（扁平标签，如 "重要的人/某朋友"、"音乐/某歌手"、"虚构角色/某作品角色"）。路径仅作为分类建议，不再创建层级节点。
...
只输出一个JSON数组，不要markdown包裹：
[{"index":0,"insight":"Clara在疲惫时倾向于用某个习惯缓解情绪，食物对她而言不只是营养更是情绪出口"},{"index":2,"insight":"Clara对家人的依赖是深层的——她可以在妈妈面前卸下所有社会面具","dimension":"emotional"},{"index":3,"insight":null}]`;
