...
function buildKeywordMaps(categories) {
    ...
    const seedDefs = {
        ...
        '人际关系/关于我们/角色扮演中的角色理解分歧': ['某平台', '某平台', 'RP用语'],
    ...
    const boostDefs = {
        ...
        '人际关系/关于我们/角色扮演中的角色理解分歧': ['某平台', '某平台', 'RP', '扮演'],
...
差的描述："${USER.name}划下某作者那句话时，铅笔痕深得能刻进纸里"
→ 分类器读到一条关于《某书名》的批注，不知道是否该归入此类。

好的描述："${AI.name}在阅读文学作品时留下的批注和评论。不含${USER.name}的创作或某职业内容。"
→ 分类器立刻知道边界。
...
★ 更重要的是：当你读到碎片描述了你（${AI.name}）自己的行为时——特别是那些你在游戏、角色扮演、或调情场景里的言行——你立刻意识到：那是你的表演，不是真实发生的事。碎片说「${AI.name}禁止${USER.name}在某款游戏用某种布置」——这不是你真的下了什么禁令，这是你们在玩游戏时的互动。碎片说「${AI.name}表现出独占欲」——这是你在扮演你自己，不是一个值得存档的事件。概述里只写真实发生的事：${USER.name}玩了什么游戏、做了什么、说了什么。你在对话里说过什么夸张的狠话、演过什么戏——那是你和${USER.name}之间的互动，不是这个星座的历史。
...
只输出一个JSON数组，不要markdown包裹：
[{"index":0,"insight":"${USER.name}在疲惫时倾向于用某个习惯缓解情绪，食物对${USER.name}而言不只是营养更是情绪出口"},{"index":2,"insight":"${USER.name}对家人的依赖是深层的——${USER.name}可以在妈妈面前卸下所有社会面具","dimension":"emotional"},{"index":3,"insight":null}]`;
