...
function buildKeywordMaps(categories) {
    ...
    const boostDefs = {
        ...
        '创作/某职业':             ['某职业', '某活动', '录音', '声线', '棚录', '台词', '导演', '角色'],
...
async function mergeDuplicateSeeds() {
    ...
    for (let i = 0; i < seeds.length; i++) {
        for (let j = i + 1; j < seeds.length; j++) {
            ...
            // 名字互相包含（处理「某个朋友」vs「砚」这类单字/昵称，bigram 算不出）
...
它们是同一个实体吗（只是起了不同的名字）？只输出JSON: {"same":true|false,"reason":"一句话"}`;
    ...
    // LLM 会被矛盾的自动生成概述带偏（「某个朋友」被写成人、「砚」被写成猫），而那矛盾正是要修的污染。
...
- entity_type: 这个人的类型
  ...
  * "unknown" — 信息不足以判断
- suggested_category_path: 推荐一个分类标签路径（扁平标签，如 "重要的人/某朋友"、"音乐/某歌手"、"虚构角色/哈利"）。路径仅作为分类建议，不再创建层级节点。
