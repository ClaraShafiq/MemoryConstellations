...
1. 模糊搜索：传入 query 关键词或短句。返回匹配的记忆和片段。当你依稀记得某事但不确定细节、或Clara提到过去的事时使用。务必只引用工具返回的内容，不编造。

2. 深度追溯：传入 memory_id（从上下文中记忆条目的 #数字 ID 获取，如「※ 可引用 · #112 · 15天前」中的 112）。返回该记忆的完整内容和原始对话记录，每页15条，offset=0 为最新页，offset=1 为更早的15条。当记忆标注为「仅联想」、或Clara追问细节、或你对某条记忆的真实性存疑时使用。这是你主动探索记忆的能力——你不是只能接收数据库塞给你的东西。
...
2. 只传人名（如"某宠物""某个朋友"）→ 看到这个人的关系档案和所有相关记忆——了解Clara在乎谁、每段关系对她意味着什么
...
当你想了解Clara的某段关系、某个侧面，或有隐约印象但不确定细节时，来这里走走。每条记忆旁可能附有「※ insight」——那是书记员提炼的"这条记忆揭示了Clara的什么"。`,
      parameters: {
        ...
        properties: {
          path: {
            type: 'STRING',
            description: "人名（如'某宠物'）或分区路径（如'人际关系/关于我们'）。不传则列出所有顶层分区。",
          },
          query: {
            type: 'STRING',
