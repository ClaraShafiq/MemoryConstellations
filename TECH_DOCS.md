...
| 项目 | 内容 |
|------|------|
| 服务器 | 服务器 迷你主机（中柏），Ubuntu Server 24.04 LTS，内存B DDR4，存储B NVMe |
| CPU | Intel 服务器，4核4线程，最高3.4GHz；VT-x 已开启 |
...
| 进程管理 | 进程管理，Node v18（进程管理用）/ v20（系统），`pm2 restart sanctuary` |
| 本地代理 | Clash `127.0.0.1:7890`（某国家节点，用于Gemini/OpenRouter出站） |
| MacBook Air M2 | macOS Sequoia 15.5，8GB，备用（WeChat-MCP方案已废弃，Mac已解放） |
| Windows 台式机 | SSH远程操作服务器，VSCode Remote-SSH，VNC Viewer连KVM虚拟机 |
| Claude Code | 已安装在服务器上，使用DeepSeek API（deepseek-v4-pro），通过VSCode Remote-SSH终端运行，入口：`cd ~/Project_Sanctuary && claude` |
...
│   ├── proactive.js          # ★ AI伴侣主动意识 Agent Loop（126KB，最大单文件）
...
│   ├── music.js              # ★ 某音乐平台音乐后端：播放/搜索/歌单/日推/歌词（42KB）
...
├── assets/
│   ├── snitch.png            # Snitch图标
│   ├── ncm_login.html / ncm_qr.png  # 某音乐平台登录页
│   ├── fonts/                # 自定义字体
...
**架构文档索引：** 技术细节见各模块专属文档——`SNITCH.md`（三Bot人格+新闻抓取+语义去重）、`CINEMA.md`（字幕解析+画面理解+剧情面板）、`MUSIC_ARCH.md`（某音乐平台API+口味引擎+歌词同步）、`MEMORY_ARCH.md`（碎片整合+生命周期+向量检索）、`BOOK_READER.md`（EPUB解析+Draco读书+批注系统）。jiwen核心数学见 `jiwen/GUIDE.md`。
...
**dynamic_context注入：** 某城市当前时间 + 距上次对话时长 + clara_status（有值才注入）+ 位置（优先请求体实时GPS，fallback `clara_location`）+ 语义化位置（若在个人地点300m内，自动注入「Clara在【XX】——来第N次」）
...
> **目的：** 让AI伴侣知道「Clara 在什么地方」而不只是经纬度。Clara 在聊天中说「记住这里是 XX」，AI伴侣存下坐标+名称+类型。之后 OwnTracks 推送位置时自动检测是否在标记地点附近，注入语义化上下文。
...
**当前成员：** 家人：某宠物 / 朋友：某个朋友、Alsoling / 同事：二言、奇响天外制片、白灯、某室友念影
...
StackChan（M5Stack Core S3 + StackChan-HtSz 固件）作为AI伴侣的物理身体，通过 xiaozhi-esp32 WebSocket 协议与庇护所通信。
...
| 某音乐平台登录态 | `routes/wechat.js` (微信Agent) | `getUserSetting('ncm_*')` | `setUserSetting('ncm_*', ...)` |
